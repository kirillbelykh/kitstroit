...
const MEDIA_CACHE_VERSION = '20260731c'
...
function ProjectMagazine({ projects }: { projects: Project[] }) {
  ...
  const [mediaMinHeight, setMediaMinHeight] = useState<number>()
  const swipeRef = useRef<{ x: number; y: number } | null>(null)
  ...
  const onMagazineImageLoad = (event: SyntheticEvent<HTMLImageElement>) => {
    const height = Math.round(event.currentTarget.getBoundingClientRect().height)
    if (height > 0) setMediaMinHeight(height)
  }
  return <section id="projects" className="projects section-ink">
    ...
    <div className="magazine">
      <div
        ...
        style={mediaMinHeight ? { minHeight: mediaMinHeight } : undefined}
      ...
      >
        <button className="magazine-open" type="button" onClick={openLightbox} aria-label={`Открыть фото ${project.title} на весь экран`}>
          <img
            ...
            src={project.media[activeMedia]}
            alt={`${project.title}, кадр ${activeMedia + 1}`}
            onLoad={onMagazineImageLoad}
          />
...
function App() {
  ...
  return <div className="site" id="top">
    ...
    <footer><a className="logo" href="#top"><span>K</span><span>I</span><span>T</span></a><p>Строительство домов и отделка квартир под ключ<br />в Санкт-Петербурге и Ленинградской области</p><p>ИП Савин Д. Н.<br />ИНН 682102074524<br />ОГРНИП 326680000024441</p><p>© 2026 KIT. Все права защищены.</p><div className="footer-links"><a href={telegramLink} target="_blank" rel="noreferrer" onClick={onTelegramClick}>Telegram</a><a href={MAX_CHANNEL} target="_blank" rel="noreferrer">Max</a><a href="/privacy">Политика конфиденциальности</a></div></footer>
